...
console.log('🚀 Starting Advanced Puppeteer Testing for dashboard...');

async function testMaskServiceDashboard() {
    let browser;
    let page;
    ...
    try {
        ...
        const backendUrl = 'http://maskservice-dashboard-backend:8202/health';
        ...
        const frontendUrl = 'http://maskservice-dashboard-frontend';
        ...
        const htmlAnalysis = await page.evaluate(() => {
            ...
            const hasCustomScript = document.querySelector(`script[src*="dashboard"]`) !== null;
            const hasCustomCSS = document.querySelector(`link[href*="dashboard"]`) !== null;
        ...
        console.log(`  Custom dashboard.js: ${htmlAnalysis.hasCustomScript ? '✅' : '❌'}`);
        console.log(`  Custom dashboard.css: ${htmlAnalysis.hasCustomCSS ? '✅' : '❌'}`);
...
// Run the test
testMaskServiceDashboard()
    .then(success => {
        console.log(`\n🏁 Testing completed for dashboard: ${success ? 'SUCCESS' : 'FAILED'}`);
