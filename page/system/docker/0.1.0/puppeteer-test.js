...
console.log('🚀 Starting Advanced Puppeteer Testing for system...');

async function testMaskServiceSystem() {
    let browser;
    let page;
    ...
    try {
        ...
        const backendUrl = 'http://maskservice-system-backend:8204/health';
        ...
        const frontendUrl = 'http://maskservice-system-frontend';
        ...
        const htmlAnalysis = await page.evaluate(() => {
            ...
            const hasCustomScript = document.querySelector(`script[src*="system"]`) !== null;
            const hasCustomCSS = document.querySelector(`link[href*="system"]`) !== null;
        ...
        console.log(`  Custom system.js: ${htmlAnalysis.hasCustomScript ? '✅' : '❌'}`);
        console.log(`  Custom system.css: ${htmlAnalysis.hasCustomCSS ? '✅' : '❌'}`);
        ...
        console.log('✅ Screenshot saved to /tmp/system-screenshot.png');
...
// Run the test
testMaskServiceSystem()
    .then(success => {
        console.log(`\n🏁 Testing completed for system: ${success ? 'SUCCESS' : 'FAILED'}`);
