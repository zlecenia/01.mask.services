...
console.log('🚀 Starting Advanced Puppeteer Testing for tests...');

async function testMaskServiceTests() {
    let browser;
    let page;
    ...
    try {
        ...
        const backendUrl = 'http://maskservice-tests-backend:8203/health';
        ...
        const frontendUrl = 'http://maskservice-tests-frontend';
        ...
        const htmlAnalysis = await page.evaluate(() => {
            ...
            const hasCustomScript = document.querySelector(`script[src*="tests"]`) !== null;
            const hasCustomCSS = document.querySelector(`link[href*="tests"]`) !== null;
        ...
        console.log(`  Custom tests.js: ${htmlAnalysis.hasCustomScript ? '✅' : '❌'}`);
        console.log(`  Custom tests.css: ${htmlAnalysis.hasCustomCSS ? '✅' : '❌'}`);
        console.log(`  Body Content Length: ${htmlAnalysis.bodyContentLength} chars`);
        
        // Take screenshot for debugging
        console.log('📸 Taking screenshot for debugging...');
        await page.screenshot({ 
            path: '/tmp/tests-screenshot.png',
            fullPage: true 
        });
        console.log('✅ Screenshot saved to /tmp/tests-screenshot.png');
...
// Run the test
testMaskServiceTests()
    .then(success => {
        console.log(`\n🏁 Testing completed for tests: ${success ? 'SUCCESS' : 'FAILED'}`);
