...
console.log('🚀 Starting Advanced Puppeteer Testing for login...');

async function testMaskServiceLogin() {
    let browser;
    let page;
    ...
    try {
        ...
        const backendUrl = 'http://maskservice-login-backend:8201/health';
        ...
        const frontendUrl = 'http://maskservice-login-frontend';
        ...
        const htmlAnalysis = await page.evaluate(() => {
            ...
            const hasCustomScript = document.querySelector(`script[src*="login"]`) !== null;
            const hasCustomCSS = document.querySelector(`link[href*="login"]`) !== null;
        ...
        console.log(`  Custom login.js: ${htmlAnalysis.hasCustomScript ? '✅' : '❌'}`);
        console.log(`  Custom login.css: ${htmlAnalysis.hasCustomCSS ? '✅' : '❌'}`);
...
// Run the test
testMaskServiceLogin()
    .then(success => {
        console.log(`\n🏁 Testing completed for login: ${success ? 'SUCCESS' : 'FAILED'}`);
